...
class ObjectRecognizer(Node):
	...
	def __init__(self):
		...
		self.allowed_objects = [
			...
			'bottle', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'banana', 'apple', 'sandwich', 
			...
			for i, det in enumerate(pred):
				if len(det):
					for *xyxy, conf, cls in reversed(det):
						...
						# ADD THIS FILTER: Only process allowed objects
						if object_name in self.allowed_objects:
							...
							# Optional: Log filtered objects for debugging
							self.get_logger().debug(f"Filtered out non-allowed object: {object_name}")
